...
- **현재 Free 플랜으로 운영한다.** 15분 idle 후 spin-down 되어 다음 요청이 1~3분 걸리지만,
  n8n 이 본 작업 전에 `/health` 로 먼저 깨우고 시작하므로 마감 시각에는 영향이 없다.
  메모리는 실측 **188MB / 512MB** 로 여유가 있다. CPU 는 0.1 이라 학습이 로컬(1초)보다 느리다.
  → `OMP_NUM_THREADS=1` 을 반드시 넣을 것. 기본값이면 XGBoost 가 호스트 코어 수만큼 스레드를
  띄워 0.1 CPU 를 두고 다투느라 오히려 느려진다. 상시 응답이 필요해지면 Starter($7/mo) 로 올린다.
- ⚠️ **스레드 수가 모델 점수를 미세하게 바꾼다.** 실측에서 M4 점수가 0.53↔0.56 움직였다
  (XGBoost 의 성질). 최종 배분은 M1 이 0.50 을 넘느냐로만 갈려 대개 같지만, 점수가 0.50
  근처인 날(과거 531일 중 ±0.02 안에 23일 = 4.3%)에는 판단이 뒤집힐 수 있다. 로컬과 Render 는
  CPU·빌드가 달라 스레드 설정과 무관하게 원래도 완전히 동일하지는 않다.
- ⚠️ **`render.yaml` 은 `New → Blueprint` 로 만들 때만 읽힌다.** `New → Web Service` 로
  만들면 무시되므로 Build/Start Command 와 환경변수를 대시보드에 직접 넣어야 한다 (DEPLOY.md §3).
...
|---|---|---|---|
| ① 예측 | `40 12 * * *` | 여름 07:40 / 겨울 06:40 CT | **DA 입찰 마감 10:00 CT 이전**에 결과가 나와야 함. Free 웨이크업 몫 20분 포함 |
| ② backfill | `0 16 * * *` | 여름 11:00 / 겨울 10:00 CT | 전일 정산 확정 후. 이 경로는 Render 를 부르지 않아 웨이크업 불필요 |
| ③ look-back | `40 16 * * 1` | 월요일 | ②가 그날 실적을 채운 40분 뒤 |

cron 은 UTC 고정이라 서머타임에 따라 현지 시각이 한 시간 움직인다. 셋 다 여유가 있다.

①과 ③의 첫 노드는 **`Wake Render (Free)`** — `/health` 를 먼저 호출해 잠든 서버를 깨운다
(타임아웃 240초). 그 뒤 `/predict`·`/score` 는 타임아웃 300초 · 재시도 4회로 호출된다.

> 재시도 설정은 n8n 에서 **노드 레벨 속성**(`retryOnFail`/`maxTries`/`waitBetweenTries`)이다.
> `parameters.options.retry` 안에 넣으면 조용히 무시된다 — 원본이 그 상태였고 2026-08-14 정정했다.
