...
**비밀값을 화면에서 일일이 넣기 번거로우면**

```bash
python push_n8n.py --init-secrets
```

`n8n_push.local.json` 에 빈칸 4개가 생긴다. 한 번만 채워 두면 **새로 만드는 워크플로도
자동으로 채워진다.** 무엇을 넣는지는 명령이 알려준다.

| 칸 | 넣을 것 |
|---|---|
| `CHANGE-ME-ERCOT-PW` | ERCOT 계정 비밀번호 |
| `CHANGE-ME-ERCOT-KEY` | ERCOT 구독 키(Subscription Key) |
| `CHANGE-ME-RENDER-API-KEY` | Render 환경변수 `API_KEY` 에 넣은 값 |
| `CHANGE-ME-CLAUDE-API-KEY` | Claude API 키 |

채우는 순서는 **n8n 이 먼저, 이 파일이 나중**이다. 화면에서 값을 바꿔도
이 파일의 옛 값이 덮어쓰지 않는다.

⚠️ 이 파일에는 진짜 비밀값이 평문으로 들어간다. `.gitignore` 에 있어 깃에는 안 올라가지만,
**PC 안에는 평문으로 남는다.** 그게 싫으면 이 기능을 쓰지 말고 n8n 화면에서 직접 넣으면 된다
(그래도 나머지 기능은 그대로 동작한다).
...
- **레포 JSON 에 진짜 비밀값이 섞이면 실행 자체가 멈춘다.** `--pull` 이나 손편집으로
  실제 값이 들어간 채 커밋되는 사고를 막는다. 값은 화면에 찍지 않고 어느 파일의
  어느 항목인지만 알려준다.
